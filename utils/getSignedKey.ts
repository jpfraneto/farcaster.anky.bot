...
const generate_signature = async function (public_key: string) {
  if (typeof GRASSCASTER_FARCASTER_DEVELOPER_MNEMONIC === "undefined") {
    throw new Error("GRASSCASTER_FARCASTER_DEVELOPER_MNEMONIC is not defined");
  }

  const FARCASTER_DEVELOPER_MNEMONIC = GRASSCASTER_FARCASTER_DEVELOPER_MNEMONIC;
  const FID = await getFid();
